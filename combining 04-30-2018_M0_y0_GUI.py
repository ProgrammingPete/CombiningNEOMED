...
def gui_selection():
    ...
    R2.pack( anchor = W)
    R3 = Radiobutton(window, text="3. To run both.", variable=var, font= TimesNewRoman, value='3')
    R3.pack(anchor = W)
    ...
    with open(file_path) as file:
        for row in file:
            ...
            if rowq_counter == 1 or rowq_counter == 2:
                ...
                row[0] = row[0] + str(rowq_counter) #creation of peptides list 
                peptides_list.append(row[0])
                peptides_dict[row[0]] = []
                data_list = []
                counter_random = 1
                for i in data_locations:   #find intensities within the csv file
                    if test_for_number(row,i)==True:
                        data_list.append(row[i])   #append colum location to data_list
                    else:
                        data_list.append(0)
                    if counter_random == 6:
                        counter_random = 0
                        (I0,I1,I2,I3,I4,I5) = time_names_fxn(data_list)   #assign to vars 
                        peptides_dict = test_for_zeros(I0,I1,I2,I3,I4,I5,peptides_dict,row) #chekc for zeros 
                        data_list = []
                    else:
                        pass
                    counter_random +=1
...
def time_names_fxn(data_list):  #extracts intensities from the data_list into separate float vars
...
def run_prism_script(prism_path):
    ...
    call([prism_path,script_path], shell=True) #WindowsError: [Error 740]
...
def read_file_prism(folder_name, row_counter):
    for file_name3 in os.listdir(folder_name):
        if 'Results.txt' in file_name3:
            ...
            for ( p,y0,pl,k,kse,h,r) in zip(peptides,_y0, plateau,k_values,k_values_se,half_life, R2): #Zip through the lists to create a dictionary of all the data for easy access
                data_dict[p] = [y0, pl,k,kse,h,r] #Dict now created with all the info. data_dict is changed on each call of this method. added y0

            

            create_data_file_individual(data_dict, peptides, file_name3, folder_name) #Create individual data file here
            protein_data, row_counter = prepare_protein_data(file_name3, data_dict, peptides, row_counter) #not sure what this does yet (6/19/18)
...
def create_data_file_individual(data_dict, peptides, file_name3, folder_name):
    ...
    data_names = ['','y0','Plateau','Rate Constant','Half-Life','K-Value SE','R^2']

    for coly in range(1,len(peptides)):
        if coly == 1:
            for rowy in range(1,8):
...
def prepare_protein_data(file_name3, data_dict, peptides, row_counter):
    #returns protien_data and current row_counter. Protien_data is ready to be processed as an CSV
    new_file_name_3 = file_name3.split('_')
    new_file_name_3 = '_'.join(new_file_name_3[0:len(new_file_name_3) - 1])
    protein_data.append([new_file_name_3])#Added protein name, this is a global array of strings
    ...
    #temp list is formed to store data in raw form. This will be appended to protien_data
    temp_listy = []
    ...
    for x in range(3,len(peptides)):
        try:
            if float(data_dict[peptides[x]][5]) < .95 or '~' in data_dict[peptides[x]][2]:
                temp_listy.append('')
            else:
    ...
    temp_listy.append('')
    for x in range(3,len(peptides)):
        temp_listy.append(data_dict[peptides[x]][5])
    protein_data.append(temp_listy)
    row_counter +=1
...
def main():   
    ...
    if selection == '1':
        ...
        run_prism_script(prism_path)            #script that was generated is now run
        read_file_prism(folder_name, row_counter) #data is extracted from the output of the prism script
